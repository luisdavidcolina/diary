...
const SyllabusChecklist = ({ items, doneMap, onToggle, onOpen, showSubject = false, showDetails = false, emptyLabel }) => {
  if (!items.length) {
  ...
  return (
    <ul className="checklist">
      {items.map((it) => {
        ...
        return (
          <li key={it.id} className={`checklist-row ${done ? 'is-done' : ''}`}>
            ...
            {showDetails && it.details?.length > 0 && (
              <ul className="checklist-details">
